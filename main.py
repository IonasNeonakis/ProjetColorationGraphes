        ...
        if deg_x < 5:
            ...
            if len(used_colors) < 5:
                ...
                if get_path(induced_subgraph_ag, a, c): # s'il y a un chemin de a vers c
                    connected_component = breadth_first_search(induced_subgraph_bd, b) # on prend la composante connexe G'(beta, delta) qui contient b
                    new_coloring = inverse_color(connected_component, coloring_res, beta, delta)# on inverse les couleurs de G'(beta, delta)
                else: # S'il y a un chemin de b à d ou qu'il n'y a pas de chemin de a à c et de b à d
                    connected_component = breadth_first_search(induced_subgraph_ag, a) # on prend la composante connexe G'(alpha, gamma) qui contient a
                    new_coloring = inverse_color(connected_component, coloring_res, alpha, gamma)# on inverse les couleurs de G'(alpha, gamma)

                remaining_colors = AVAILABLE_COLORS - neighbourhood_colors(neighbours, new_coloring) # on récupère la couleur restante
